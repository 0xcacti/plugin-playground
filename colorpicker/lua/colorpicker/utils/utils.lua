...
M.HSLToRGB = function(h, s, l)
    ...
    local function to(p, q, t)
        ...
        if t < 0.16666666666667 then
            return p + (q - p) * 6 * t
        end
        if t < 0.5 then
            return q
        end
        if t < 0.66666666666667 then
            return p + (q - p) * (0.66666666666667 - t) * 6
        end
        return p
    end
    local q = l < 0.5 and l * (1 + s) or l + s - l * s
    local p = 2 * l - q
    return { M.round(to(p, q, h + 0.33333333333333) * 255), M.round(to(p, q, h) * 255), M.round(to(p, q,
        h - 0.33333333333333) * 255) }
end
